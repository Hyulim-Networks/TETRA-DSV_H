...
#include "tetraDS_service/set_marker10.h" //SRV 
#include "tetraDS_service/set_marker20.h" //SRV 
...
//Lift Service
#include "tetraDS_service/gotolift.h" //SRV
#include "tetraDS_service/liftingcheck.h" //SRV
#include "tetraDS_service/unliftingcheck.h" //SRV
...
#include "tetraDS_service/patrol_lift.h" //SRV ... 231123
#include "tetraDS_service/lift_auto_movement.h" //SRV ... 231123
...
{
    string m_iAR_tag_frame = "TETRA_NS/usb_cam";
...
tetraDS_service::set_marker10 set_marker10_cmd; 
ros::ServiceServer set_marker10_service; 
tetraDS_service::set_marker20 set_marker20_cmd;
ros::ServiceServer set_marker20_service; 
...
//Lift Service
tetraDS_service::gotolift gotolift_cmd;
ros::ServiceServer gotolift_service;
tetraDS_service::liftingcheck liftingcheck_cmd;
ros::ServiceServer liftingcheck_service;
tetraDS_service::unliftingcheck unliftingcheck_cmd;
ros::ServiceServer unliftingcheck_service;
//patrol On / Off Service//
tetraDS_service::patrol patrol_cmd;
ros::ServiceServer patrol_service;

//patrol Lift Service
tetraDS_service::patrol_lift patrol_lift_cmd;
ros::ServiceServer patrol_lift_service;
//lift movement Service Client//
ros::ServiceClient lift_cmd_client;
tetraDS_service::lift_auto_movement lift_srv;
...
//Front Bumper_data to Pointcloud2_data
...
//Rear Bumper_data to Pointcloud2_data//
...
//lift_test...
int  m_iLifting_ID = 0;
int  m_iUnlifting_ID = 0;
string m_strLifting_lobcation_name = "LIFTING";
string m_strUnlifting_lobcation_name = "UNLIFTING";
...
void Dynamic_reconfigure_AR_marker_size_Set_DoubleParam(string strname, double dValue) //ar_marker_dynamic_reconfigure 
{
    dynamic_reconfigure::ReconfigureRequest srv_req;
    dynamic_reconfigure::ReconfigureResponse srv_resp;
    dynamic_reconfigure::DoubleParameter double_param;
    ...
    ros::service::call("ar_track_alvar/set_parameters", srv_req, srv_resp);
}
...
bool Un_Depart_10mm_Liftmove() // not use cygbot lidar because backward move in port 
{
    if(_pRobot_Status.m_iCallback_Bumper = 1)
    {
        printf("GO Depart 10mm OUT OF LIFT PORT \n");
...
bool LiftMovement1(ros::ServiceClient& lift_cmd_client)
{
    ...
    if (lift_cmd_client.call(lift_srv))
    {
        ...
        while(_pRobot_Status.LIFT_MOVEMENT>0){
            ros::spinOnce();
        }
...
bool LiftMovement2(ros::ServiceClient& lift_cmd_client)
{
    ...
    if (lift_cmd_client.call(lift_srv))
    {
        ...
        while(_pRobot_Status.LIFT_MOVEMENT<240){
            ros::spinOnce();
        }
        sleep(3); //wait 3sec
...
bool Depart_1500mm_Liftmove(int mode) // not use cygbot lidar because backward move in port 
{
    ...
    geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
    ros::Time start_time = ros::Time::now();
    ros::Duration duration(time); //define duration seconds
    ros::Rate rate(10); // 10Hz
				  ...
				  tetraDS_service::gotolocation::Response &res)
{
    ...
    if(_pRobot_Status.m_iCallback_Charging_status <= 1 && (_pAR_tag_pose.m_iAR_tag_id == -1 || _pAR_tag_pose.m_transform_pose_x >= 0.4)) 
    {
...
{
    if(_pRobot_Status.m_iCallback_Charging_status != msg->data){
        ROS_INFO("_pRobot_Status.m_iCallback_Charging_status != msg->data %d", msg->data);
    }
...
void LiftCallback(const std_msgs::Int32::ConstPtr& msg)
{
    _pRobot_Status.LIFT_MOVEMENT = msg->data; 
}
...
void setARGoal(double target_dist)
{
    ...
    goal_pose.pose.position.x += offset * cos(-_pAR_tag_pose.m_fAR_tag_pitch-(-0.044*target_dist+0.1022));
    goal_pose.pose.position.y += offset * sin(-_pAR_tag_pose.m_fAR_tag_pitch-(-0.044*target_dist+0.1022));
...
void AR_tagCallback(ar_track_alvar_msgs::AlvarMarkers req) 
{
    if (!req.markers.empty()) 
    {
        ...
        //AR_Tag data update...
        _pAR_tag_pose.m_iAR_tag_frame = req.markers[_pAR_tag_pose.m_iAR_tag_id_Index].header.frame_id;
        ...
        _pAR_tag_pose.m_fAR_tag_pitch+=0.02;
...
bool ChargingStation_tracking(bool bOn, int marker_id)
{
    ...
    if(bOn)
    {
        ...
        if(_pAR_tag_pose.m_iAR_tag_id == marker_id)
        {
            ...
            {
                cmd->linear.x = 1.0 * m_fdistance * 0.2; // 0.15 
...
bool ChargingStation_tracking2(int marker_id)
{
    ...
    float m_fdistance = 0.0;
        if(_pRobot_Status.m_iCallback_Charging_status < 2&&_pRobot_Status.m_iCallback_Bumper == 0) 
        {
            m_fdistance = sqrt(_pAR_tag_pose.m_transform_pose_x * _pAR_tag_pose.m_transform_pose_x + _pAR_tag_pose.m_transform_pose_y * _pAR_tag_pose.m_transform_pose_y);

            cmd->linear.x = 1.0 * (m_fdistance /2.0) * 0.05; // max speed 0.1m/s
            if(cmd->linear.x > 1.0)
            {
                //Linear Over speed exit loop......
                cmd->linear.x =  0.0; 
...
/****************************************************************/
//Lift Station Docking Function//
bool LiftStation_tracking(bool bOn, int marker_id)
{
    bool bResult = false;
    geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
    ...
    if(bOn)
    {
        ...
        if(_pAR_tag_pose.m_iAR_tag_id == marker_id)
        {
            m_fdistance = sqrt(_pAR_tag_pose.m_transform_pose_x * _pAR_tag_pose.m_transform_pose_x + _pAR_tag_pose.m_transform_pose_y * _pAR_tag_pose.m_transform_pose_y);
            printf("master_distance: %.5f \n", m_fdistance);
            if(m_fdistance > 1.6 && m_fdistance < 3.0) 
            {
...
{ 
    ...
    if(_pAR_tag_pose.m_target_yaw <= 0.0174533 && _pAR_tag_pose.m_target_yaw >= -0.0174533) 
    {
        ex_iDocking_CommandMode = 15;
        bResult = true;
        return bResult;
    }
    ...
    {
        printf("--dir \n");
        cmd->angular.z = -1.0 * _pAR_tag_pose.m_target_yaw * 1.2; 
        cmdpub_.publish(cmd);
        sleep(2);

        if(m_fdistance > 3.0 || m_fdistance < -3.0) 
        {
...
bool LiftStation_tracking2(int marker_id)
{
    bool bResult = false;
    geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
...
bool Un_LiftStation_tracking(bool bOn, int marker_id)
{
    ...
    if(bOn)
    {
        ...
        if(_pAR_tag_pose.m_iAR_tag_id == marker_id)
        {
            ...
            if(m_fdistance > 1.6 && m_fdistance < 3.0) 
            {
                cmd->linear.x = 1.0 * m_fdistance * 0.02; 
                printf("linear velocity: %.2f \n", cmd->linear.x);
                if(cmd->linear.x > 1.0)
                {
                    //Linear Over speed exit loop...
...
bool Goto_Lift_Command(tetraDS_service::gotolift::Request &req, 
				            tetraDS_service::gotolift::Response &res)
{
	bool bResult = false;
    ...
    goto_goal_id.id = req.Location;
    _pRobot_Status.LIFT_ID = req.id;
    //_pRobot_Status.LIFT_MOVEMENT = req.movement;
    _pFlag_Value.m_bflag_Lift_docking = true;

    ROS_INFO("goto_lift_name %s, id %d, movement %d", goto_goal_id.id.c_str(), _pRobot_Status.LIFT_ID, _pRobot_Status.LIFT_MOVEMENT);
...
bool Lifting_check_Command(tetraDS_service::liftingcheck::Request &req, tetraDS_service::liftingcheck::Response &res) 
{
	bool bResult = false;
    //to do Check Loop...
    if(_pRobot_Status.m_iLift_Sensor_info == 0)
    {
        bResult = true;
    }
    else
...
bool Unlifting_check_Command(tetraDS_service::unliftingcheck::Request &req, tetraDS_service::unliftingcheck::Response &res)
{
	bool bResult = false;
    //to do Check Loop...
    if(_pRobot_Status.m_iLift_Sensor_info >= 2)
    {
        bResult = true;
    }
    else
    ...
    while(1)
    {
        switch(ex_iDocking_CommandMode)
        {
            ...
            case 6:
                ...
                break;
            case 7:
                if(_pAR_tag_pose.m_fAR_tag_pitch <= 0.0104533 && _pAR_tag_pose.m_fAR_tag_pitch >= -0.0104533) //+- 1.0deg
                {
                    ex_iDocking_CommandMode = 2;
                }else{
                    setARGoal(1.3);
            ...
            case 11:
                ...
                sleep(2); //wait 2sec
                ex_iDocking_CommandMode = 18;
                docking_progress.data = 11;
                docking_progress_pub.publish(docking_progress);

                break;
            case 12:
                LiftStation_tracking(true, _pAR_tag_pose.m_iSelect_AR_tag_id);
            ...
            case 14:
                ...
                docking_progress.data = 14;
                docking_progress_pub.publish(docking_progress);
                break;
            case 15:
                //charging_port_On_client.call(m_request2);
                //pproach_Station2Move2();
                LiftMovement1(lift_cmd_client);
                Lift_Check();
                if(_pFlag_Value.m_bfalg_DockingExit)
                {
    ...
    while(1)
    {
        if(_pFlag_Value.m_bflag_patrol)
        {
            ...
            if(_pRobot_Status.m_iCallback_Battery <= LOW_BATTERY)
            {
                ...
                LED_Toggle_Control(1,3,100,3,1);
                LED_Turn_On(100);
                ROS_INFO("Battery Low - 15 percent  come back Home [While autothread] ");
...
void resultCallback(const move_base_msgs::MoveBaseActionResult::ConstPtr& msgResult)
{
  ...
  { 
    ...
    if(_pFlag_Value.m_bflag_Lift_docking) 
    {
    ...
    if(docking_progress.data == 1){
        ex_iDocking_CommandMode = 2;
    }else if(docking_progress.data == 11){
        ex_iDocking_CommandMode = 12;
    }else if(docking_progress.data == 21){
        ex_iDocking_CommandMode = 22;
    }
  ...
  else if( msgResult->status.status == ABORTED)
  {
    ...
    else
    {
        ...
        LED_Toggle_Control(1, 3,100,3,1);
        if(_pFlag_Value.m_bflag_Conveyor_docking || _pFlag_Value.m_bflag_Lift_docking) 
            LED_Turn_On(45); //sky_blue
        else
            LED_Turn_On(63);
        ...
        if(docking_progress.data == 1){
            setARGoal(1.3);
        }else if(docking_progress.data == 11 || docking_progress.data == 21 ){
            setARGoal(2.3);
        }else{
            setGoal(goal);
        }
...
int main (int argc, char** argv)
{
    ...
    goal_pub_ = nh.advertise<geometry_msgs::PoseStamped>("goal_pub",10);

    //AR_TAG_subscriber//
    ros::Subscriber AR_sub = nh.subscribe("corrected_ar_markers", 100, AR_tagCallback);
    //Laser Scan subscriber//
    ros::Subscriber scan_sub = nh.subscribe("scan", 100, LaserScanCallback);
    ros::Subscriber scan_b_sub = nh.subscribe("scan_b", 100, LaserScanCallback_b); 
    //Laser Scan Merge subscriber
    ...
    //Set_marker Service//
    set_marker10_service = service_h.advertiseService("set_marker10_cmd", set_marker10_Command);
    set_marker20_service = service_h.advertiseService("set_marker20_cmd", set_marker20_Command);
    //Patrol Service//
    ...
    patrol_lift_service = service_h.advertiseService("patrol_lift_cmd", Patrol_lift_Command);
    //Delete Data All Service//
    ...
    //Lift Move Client// 
    lift_cmd_client = client_h.serviceClient<tetraDS_service::lift_auto_movement>("Auto_Move_cmd");
    //IMU Service Client//
    ...
    ros::Subscriber lift_status = nInfo.subscribe<std_msgs::Int32>("lift_movement", 1, LiftCallback);
